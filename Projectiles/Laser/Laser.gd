...
const SCENE_PATH: String = "res://Projectiles/Laser/laser.tscn"
const SPLIT_SCALE: Vector2 = Vector2(0.5, 0.5)
...
func _handle_split() -> void:
	...
	for count in split_count:
		_create_split_laser(angle * count)

	FactoryProjectile.return_projectile(self)
func _create_split_laser(angle: float) -> void:
	var new_laser: Laser = duplicate()
	new_laser.velocity = velocity
	new_laser.position = position
	new_laser.scale = SPLIT_SCALE
	new_laser.set_collision_mask_value(3, false)
	new_laser.set_direction(Vector2(cos(deg_to_rad(angle)), sin(deg_to_rad(angle))))
